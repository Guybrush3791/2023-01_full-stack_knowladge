...
## Todo
Create un nuovo progetto Laravel. Concentratevi sul layout: *due partials* per `header` e `footer` e *un layout* per la gestione del codice `HTML` comune a tutte le pagine. 

Svolgere, come visto a lezione, due pagine, tramite 2 diverse rotte. Ogni singolo elemento portera' alla stessa pagina del dettaglio degli oggetti.
...
### Assets
#### Zip
![[2023-01-30-laravel-comics-assets.zip]]
...
### Data
E' possibile utilizzare il seguente `array` in *PHP* come fonte di dati da incollare all'interno dell'`HTML`
```php
<?php
